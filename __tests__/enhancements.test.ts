...
import {
  ...
  extractReExports,
} from "../src/splitter/workspace/reExportChainResolver";
...
describe("cognitiveComplexityExact", () => {
  ...
  it("nesting multiplier: nested if costs more than flat", () => {
    ...
    expect(cognitiveComplexityExact(nested)).toBeGreaterThan(
      cognitiveComplexityExact(flat),
    );
  });
...
describe("calibratePerFunction", () => {
  ...
  it("region at median effort gets threshold equal to file threshold", () => {
    ...
    expect(
      Math.abs(medianC.regionThreshold - medianC.fileThreshold),
    ).toBeLessThan(0.1);
  });
  ...
  it("all thresholds are within [0.10, 0.75]", () => {
    ...
    for (const [, cal] of result.calibrations) {
      expect(cal.regionThreshold).toBeGreaterThanOrEqual(0.1);
  ...
  it("user bias shifts all calibrations in the same direction", () => {
    ...
    const low = calibratePerFunction(metricsMap, 0.2);
    const high = calibratePerFunction(metricsMap, 0.6);
    expect(high.calibrations.get("r")!.regionThreshold).toBeGreaterThan(
      low.calibrations.get("r")!.regionThreshold,
    );
  });
...
const NAMESPACE_TYPE_SRC = `
import * as React from 'react';
export type WidgetProps = React.FC<{ value: string }>;
export const render = () => React.createElement('div', null);
`.trim();

const OBJECT_WITH_NAMESPACE_IMPORT = `
import * as React from 'react';
...
export const widgets = {
    render: () => {
        const tag = 'div';
        const props = null;
        const el = React.createElement(tag, props);
        const ready = true;
        if (!ready) {
...
describe("applyFunctionLevelSplit", () => {
  it("does not split a simple single-function region", () => {
    ...
    const result = applyFunctionLevelSplit(
      regions,
      symbolTable,
      SINGLE_FN,
      "f.ts",
  ...
  it("handles empty regions array", () => {
    const result = applyFunctionLevelSplit(
      [],
      { locals: new Map(), imports: new Map(), unresolved: new Set() },
      "",
      "f.ts",
    );
...
describe("namespace import handling", () => {
  it("tracks namespace alias for qualified type names", () => {
    const { regions } = parseSourceFile(NAMESPACE_TYPE_SRC, "widget.tsx");
    const typeRegion = regions.find((r) => r.name === "WidgetProps");
    expect(typeRegion).toBeDefined();
    expect(typeRegion!.usedSymbols.has("React")).toBe(true);
    expect(typeRegion!.usedSymbols.has("FC")).toBe(false);
  });

  it("does not treat namespace members as standalone symbols in sub-regions", () => {
    const { regions, symbolTable } = parseSourceFile(
      OBJECT_WITH_NAMESPACE_IMPORT,
      "widgets.tsx",
    );
    const result = applyFunctionLevelSplit(
      regions,
    ...
    const renderRegion = result.find((r) => r.name === "render");
    expect(renderRegion).toBeDefined();
    expect(renderRegion!.usedSymbols.has("React")).toBe(true);
    expect(renderRegion!.usedSymbols.has("createElement")).toBe(false);
  });
});
...
describe("SemanticTypeResolver", () => {
  ...
  it("classifies class as both type and value (dual)", () => {
    ...
    if (info.resolved) {
      ...
      expect(
        info.dualSymbols.has("ApiService") ||
          info.valueSymbols.has("ApiService"),
      ).toBe(true);
    }
  });
